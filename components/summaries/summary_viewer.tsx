...
import NavigationControles from "./NavigationControles";
...
export default function SummaryViewer({summary}:{summary:string}) {
  ...
  const handleSelectionSelect = (index:number) => setCurrentSection(Math.min(Math.max(index , 0), sections.length-1))


  
  return (
    <Card className="relative px-2 h-[500px] sm:h-[600px] lg:h-[700px] w-full xl:w-[600px] overflow-hidden bg-linear-to-br from-background via-background/95 to-green-500/5 backdrop-blur-lg shadow-2xl rounded-3xl border border-green-500/10">
      ...
      <div className="h-full overflow-y-auto scrollbar-hidden pt-12 sm:pt-16 pb-20 sm:pb-24">
        ...
        <NavigationControles currentSection={currentSection} totalSections={sections.length} onPrevious={handlePrevious} onNext={handleNext} onSectionSelect={handleSelectionSelect} />
    </Card>
  )
}
