        ...
        <div className="px-4 flex gap-1.5">
            {sections.map((_, index)=>( 
                <div key={index} className='h-1.5 flex-1 rounded-full bg-rose-500/10 overflow-hidden'>
                    <div  className={cn('h-full bg-linear-to-r from-gray-500 to-green-600 transition-all duration-500',index === currentSection ? 'w-full' : currentSection>index ? 'w-full opacity-10':'w-0')}/>
                </div>
            ))}
        </div>
    </div>
